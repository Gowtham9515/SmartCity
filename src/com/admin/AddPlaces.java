...
/**
 * Servlet implementation class AddPlaces
 */
		...
		if (ServletFileUpload.isMultipartContent(request)) {
			try {
				...
				try {
					...
					ResultSet resultset = DatabaseConnection.getResultFromSqlQuery("select * from tblplaces where place='" + place + "'");
					if (resultset.next()) {
						String message = "Place already exist";
						session.setAttribute("already-exist", message);
						response.sendRedirect("admin-add-places.jsp");
					} else {
						int i = DatabaseConnection.insertUpdateFromSqlQuery(
								"insert into tblplaces(pid,category,place,state,city,address,description,image,image_path) values('"
