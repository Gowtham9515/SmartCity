...
/**
 * Servlet implementation class AdminEditPlace
 */
@WebServlet("/AdminEditPlace")
public class AdminEditPlace extends HttpServlet {
			...
			try {
				...
				try {
					...
					if (imageName == null || imageName.isEmpty()) {
						...
						int i = DatabaseConnection.insertUpdateFromSqlQuery("update tblplaces set category='" + category + "',place='" + place + "',address='"+ address + "',description='" + description + "' where pid='" + pid + "'");
						if (i > 0) {
							String success = "Place edit successfully.";
							session.setAttribute("addedMessage", success);
							response.sendRedirect("admin-view-place-details.jsp");
						}
					} else {
						
						int i = DatabaseConnection.insertUpdateFromSqlQuery("update tblplaces set category='" + category+ "',place='" + place + "',address='" + address + "',description='" + description+ "',image='" + imageName + "',image_path='" + imagePath + "' where pid='" + pid + "'");
						if (i > 0) {
							String success = "Place edit successfully.";
							session.setAttribute("addedMessage", success);
							response.sendRedirect("admin-view-place-details.jsp");
